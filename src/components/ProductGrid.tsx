...
const ProductGrid = () => {
  ...
  return (
    <section ref={gridRef} className="py-24 lg:py-32 bg-background relative overflow-hidden">
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-secondary/8 rounded-full blur-3xl animate-pulse" />
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 gap-8">
          <div>
            <div className="inline-flex items-center gap-3 px-5 py-2 bg-secondary rounded-full mb-6 shadow-blood-glow animate-float">
              <span className="text-white text-2xl animate-glow-pulse">★</span>
            ...
            <h2 className="text-display font-bold leading-none text-primary">
              Nos
              <span className="block italic text-secondary">essentiels</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 lg:gap-6 auto-rows-[280px]">
          {products.map((product, index) => {
            ...
            return (
              ...
              >
                <div className={`relative rounded-2xl lg:rounded-3xl overflow-hidden bg-card h-full shadow-void hover:shadow-void-lg transition-all duration-500`}>
                  <img
                    src={product.image}
                    alt={product.title}
                  ...
                  {/* Gradient Overlay - Fusion Noir/Rouge/Ocre */}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 lg:p-6 transform translate-y-0 transition-transform duration-500">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button className="w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-secondary text-white flex items-center justify-center hover:scale-110 transition-all shadow-blood-glow">
                  ...
                  {product.tag && (
                    <div className="absolute top-3 left-3 lg:top-4 lg:left-4 px-3 py-1.5 lg:px-4 lg:py-2 backdrop-blur-md bg-accent border border-accent rounded-full shadow-desert-glow animate-float">
                      <span className="text-xs font-bold text-primary">NEW</span>
                    </div>
                  )}
                  
                  {index === 0 && (
                    <div className="absolute top-3 right-3 lg:top-4 lg:right-4 w-12 h-12 lg:w-16 lg:h-16 rounded-full bg-secondary text-white flex items-center justify-center text-xs lg:text-sm font-bold shadow-blood-glow animate-pulse">
                      TOP
                    </div>
                  )}
                </div>
        ...
        <div className="mt-16 flex justify-center">
          <button className="group relative px-12 py-5 rounded-full bg-secondary text-white hover:scale-105 transition-all flex items-center gap-4 shadow-blood hover:shadow-blood-glow font-bold text-lg overflow-hidden">
            <span className="relative z-10">Voir tout</span>
            <ArrowRight className="w-6 h-6 group-hover:translate-x-2 transition-transform relative z-10" />
            <div className="absolute inset-0 bg-secondary-light translate-x-[-100%] group-hover:translate-x-0 transition-transform duration-500"></div>
          </button>
        </div>
      </div>
    </section>
